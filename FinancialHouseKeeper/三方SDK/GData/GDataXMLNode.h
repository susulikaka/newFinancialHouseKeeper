...
/* 使用说明:需要导入的库libxml2.dylib */
/* tips:使用libxml2.dylib需要修改项:
   Target－build setting --> header search path --> ${SDKROOT}/usr/include/libxml2
*/
/*
 * GDataXMLDocument * doc = [[GDataXMLDocument alloc]initWithXMLString:string options:1 error:nil]; // 新建解析对象
 * GDataXMLElement * element = doc.rootElement; // 获取根结点
 
 * elementForName:获取当前层级下某个键的所有子结点
 * children:获取当前层级所有子结点
 * childCount:获取当前层级所有子结点个数
 * nodesForXPath:获取路径下的所有子结点
      @"/one/two/three.....":获取 "特定层级路径" 下的 "特定结点" 的所有元素 
      @"///three":获取 "任意路径" 下 "特定层级" 包含 "特定结点" 的所有元素
 
 * stringValue:当前结点对象的值（NSString类型）
 * name:当前结点对象的键
 */
